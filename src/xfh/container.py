...
from xfh.errors import CorruptDataError, InvalidFormatError, ResourceLimitError, XfhError
from xfh.limits import Limits
from xfh.models import ChunkInfo, FileFormat, FileInfo, RecoveryIssue
...
def parse(data: bytes, limits: Limits, *, salvage: bool = False) -> ParsedFile:
    """Parse and validate one complete stream."""

    format_ = detect_format(data)
    if format_ is FileFormat.XPKF:
        return _parse_xpkf(data, limits, salvage=salvage)
    return _parse_legacy(data, limits, salvage=salvage)
...
def _parse_xpkf(data: bytes, limits: Limits, *, salvage: bool = False) -> ParsedFile:
    if len(data) < 36:
    ...
    if total_size != len(data) and not salvage:
    ...
    try:
        while offset < len(data):
            ...
            next_offset = payload_end + ((-packed_size) & 3)
            ...
            if unpacked_total + raw_size > unpacked_size:
                raise CorruptDataError("XPKF chunks exceed declared output size")
        ...
        if total_size != input_size:
            raise CorruptDataError(
                f"XPKF length field describes {total_size} bytes, input has {input_size}"
            )
    except XfhError as error:
        if not salvage:
            raise
        issues.append(RecoveryIssue(offset, str(error)))
    ...
    return ParsedFile(file_info, tuple(chunks), tuple(issues))


def _parse_legacy(data: bytes, limits: Limits, *, salvage: bool = False) -> ParsedFile:
    if len(data) < 32:
    ...
    try:
        for index, (start, end) in enumerate(pairwise(boundaries)):
            ...
            if chunk_type == 0:
                ...
                raise CorruptDataError(
                    f"legacy chunk {index} length does not match its table boundary"
                )
        ...
        if not salvage:
            raise
        issues.append(RecoveryIssue(start, str(error)))
